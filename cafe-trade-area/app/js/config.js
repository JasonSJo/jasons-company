...
const CFG = (() => {
  ...
  const SPEC = {
    ...
    상업용_연임대수익률: [0.045, ESTIMATED, 콘솔, 'M5', '지역 매매 시세를 기대 임대료로 환산할 때 쓰는 연 수익률. 실거래가는 매매가이므로 임대 조건과 직접 비교할 수 없어 이 계수가 필요하다', 0.01, 0.15, 0.001],
    시세대비_보류배수:   [2.0,  ESTIMATED, 콘솔, 'M5', '제시 임대료가 지역 시세 기대치의 이 배수를 넘으면 보류. 부결이 아니라 보류다 — 근거가 약한 신호로 사람을 떨어뜨리지 않는다', 1, 5, 0.1],
    시세대조_최소건수:   [5,    ESTIMATED, 콘솔, 'M5', '지역 실거래가 이 건수 미만이면 대조하지 않는다. 표본이 적으면 중앙값이 한두 건에 끌려다닌다', 1, 50, 1],
